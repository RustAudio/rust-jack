...
fn print_src(fns: &[Function]) {
    println!("use crate::types::*;");
    println!("use lazy_static::lazy_static;");
    println!("pub struct JackFunctions {{");
    println!("    pub library: libloading::Library,");
    for f in fns.iter() {
    ...
    println!("lazy_static! {{");
    println!("    static ref LIB: JackFunctions = unsafe {{");
    println!("        let library = libloading::Library::new(\"libjack.so.0\").unwrap();");
    for f in fns.iter() {
        ...
        if f.flags.contains(FunctionFlags::WEAK) {
            ...
            println!("        let {}_impl = {}_impl.into_raw();", f.name, f.name);
            println!(
    ...
    println!("        JackFunctions {{");
    println!("            library,");
    for f in fns.iter() {
        println!("            {}_impl,", f.name);
    }
    println!("        }}");
    println!("    }};\n");
    println!("}}\n");
